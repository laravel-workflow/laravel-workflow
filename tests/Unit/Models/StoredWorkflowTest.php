...
final class StoredWorkflowTest extends TestCase
{
    public function testModel(): void
    {
        ...
        $this->assertSame(
            $workflow->exceptions()->first()->id,
            StoredWorkflowException::whereStoredWorkflowId($workflow->id)->first()->id
        );

        $this->assertSame(1, $workflow->logs()->count());
        $this->assertSame($workflow->id, $workflow->logs()->first()->stored_workflow_id);
        $this->assertSame(
            $workflow->logs()->first()->id,
            StoredWorkflowLog::whereStoredWorkflowId($workflow->id)->first()->id
        );

        $this->assertSame(1, $workflow->signals()->count());
        $this->assertSame($workflow->id, $workflow->signals()->first()->stored_workflow_id);
        $this->assertSame(
            $workflow->signals()->first()->id,
            StoredWorkflowSignal::whereStoredWorkflowId($workflow->id)->first()->id
        );

        $this->assertSame(1, $workflow->timers()->count());
        $this->assertSame($workflow->id, $workflow->timers()->first()->stored_workflow_id);
        $this->assertSame(
            $workflow->timers()->first()->id,
            StoredWorkflowTimer::whereStoredWorkflowId($workflow->id)->first()->id
        );
    }
}
